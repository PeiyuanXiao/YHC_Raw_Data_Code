...
YHDDATA_chunks <- 
  ...
  mutate(Th = as.double(Th)) %>%
  mutate(W = as.double(W))
Tektite_X_ray <- read_excel("X_ray_analysis.xlsx", sheet = 1)
Sandstone_X_ray <- read_excel("X_ray_analysis.xlsx", sheet = 2)
...
# Tektite X-ray analysis--------------------------------------------------------
chem_vars <- c("SiO2", "Fe2O3", "Al2O3", "CaO", "K2O", "TiO2", "SO3", "MnO", "Tm2O3")

pca_data <- Tektite_X_ray %>%
  select(all_of(chem_vars), Location) %>%
  filter(complete.cases(across(all_of(chem_vars))))  

pca_result <- PCA(pca_data %>% select(all_of(chem_vars)), scale.unit = TRUE, graph = FALSE)

pca_ind <- pca_result$ind$coord %>%
  as.data.frame() %>%
  mutate(Location = pca_data$Location)
...
p_tek <- ggplot(pca_ind, aes(x = Dim.1, y = Dim.2, color = Location)) +
  geom_point(size = 3, alpha = 0.8) +
  stat_ellipse(type = "norm", level = 0.95, size = 0.5, 
               linetype = "solid", segments = 200) +  
  ...
  scale_color_manual(values = c("#6495ED", "#FF7F50")) +
  theme(
...
ggsave("PCA_plot_tek.png", plot = p_tek, width = 8, height = 6.5, dpi = 600)
...
pca_ind$Location <- as.factor(pca_ind$Location)

permanova_result <- adonis2(pca_ind[, 1:3] ~ Location, data = pca_ind, method = "euclidean", permutations = 9999)
print(permanova_result)

dist_matrix <- dist(pca_ind[, 1:3], method = "euclidean")

dispersion <- betadisper(dist_matrix, group = pca_ind$Location)

permdisp_result <- permutest(dispersion, permutations = 9999)

print(permdisp_result)

# Sandstone X-ray analysis------------------------------------------------------
chem_vars <- c("Si", "Fe", "K", "Ca", "S", "Ti", "Mn", "Cu", "Cr")

pca_data <- Sandstone_X_ray %>%
  select(all_of(chem_vars), Location) %>%
  filter(complete.cases(across(all_of(chem_vars))))  
...
p_sand <- ggplot(pca_ind, aes(x = Dim.1, y = Dim.2, color = Location)) +
  geom_point(size = 3, alpha = 0.8) +
  stat_ellipse(type = "norm", level = 0.95, size = 0.5, linetype = "solid", segments = 200) + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") + 
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") + 
  labs(
    title = NULL,
...
ggsave("PCA_plot_sand.png", plot = p_sand, width = 8, height = 6.5, dpi = 600)
...
kruskal_result <- kruskal.test(L ~ `Raw material`, data = YHDDATA_complete_flakes)
print(kruskal_result)

dunn_result <- dunnTest(L ~ `Raw material`, data = YHDDATA_complete_flakes, method = "bonferroni") 
print(dunn_result)

bipolar_data <- YHDDATA_complete_flakes %>%
  filter(Technique == "Bipolar")
kruskal_result <- kruskal.test(L ~ `Raw material`, data = bipolar_data)
print(kruskal_result)
dunn_result <- dunnTest(L ~ `Raw material`, data = bipolar_data, method = "bonferroni")
print(dunn_result)
=======
# library R packages------------------------------------------------------------
library("readxl")
